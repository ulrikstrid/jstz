import {Address, isAddress, Kv, Ledger, Contract } from "./jstz_api"
...
export type UpdateOperator = { operation : "add_operator" | "remove_operator" , owner : Address, operator : Address }
export function isUpdadeOperator(argument : unknown) : argument is UpdateOperator {
  let update = argument as UpdateOperator;
  try {
    return (update.operation === "add_operator" || update.operation === "remove_operator") &&
           isAddress(update.owner) && isAddress(update.operator)
  } catch {return false}
}


export type BalanceRequest = {owner : Address, token_id : TokenId};
export function isBalanceRequest(argument : unknown) : argument is BalanceRequest {
...
export function isBalanceResponse(argument : unknown) : argument is BalanceResponse {
...
function transferTokens(from : Address, to : Address, tokenId : TokenId, amount : number) {
  changeBalance(from, tokenId, -amount);
  changeBalance(to, tokenId, amount);
}

function operatorKey(owner : Address, operator : Address) : string {
  return `owner/${owner}/${operator}`;
}
function setOperator(owner : Address, operator : Address) {
  Kv.set(operatorKey(owner, operator), true);
}
function unsetOperator(owner : Address, operator : Address) {
  Kv.delete(operatorKey(owner, operator));
}
function assertOperator(owner : Address, operator : Address) {
  if (!(owner === operator || Kv.get(operatorKey(owner, operator)))) {
...
function performTransfer(from : Address, transfer : Transfer) {
...
function performTransfers(referer: Address, transfers : Transfers) {
  assertOperator(transfers.from, referer);
  transfers.transfers.forEach((transfer) => performTransfer(transfers.from, transfer))
}
...
function performUpdateOperator(referer: Address, update: UpdateOperator) {
  switch (update.operation) {
      case "add_operator":
        ...
        setOperator(update.owner, update.operator);
        break;
      case "remove_operator":
        assertOperator(update.owner, referer);
        unsetOperator(update.owner, update.operator)
  }
}
function forEachUnique<T,R>(ts : T[], f: (t:T) => R) : R[] {
  let seen = {};
  let output = [];
  ts.forEach((t : T) => {
    let key = t.toString();
...
async function performBalanceOf(balanceOf : BalanceOf) : Promise<BalanceResponse[] > {
        ...
        switch (path) {
            ...
            case "/balance_of":
                if (request.method === "GET") {
                    let balanceOf = {
                        // TODO: This won't work bacause you can't send json as plaintext in urls
            ...
            case "/update_operators":
                if (request.method === "PUT") {
                  let update = request.json();
                  if (isUpdadeOperator(update)) {
                    performUpdateOperator(request.headers.get("Referrer"), update);
                  } else {
                    console.error(`Invalid parameters ${update}`);
                    return Response.error();
                  }
                } else {
